...
export async function GET(
  ...
  const deadlines = db
    .prepare("SELECT * FROM deadlines WHERE case_id = ? ORDER BY due_date")
    .all(id) as Array<{
    title: string;
    due_date: string;
  }>;
  ...
  // Header
  y = drawText("BUREAUCRACY BUSTER", margin, y, { font: fontBold, size: 20, color: navy });
  ...
  y = drawText(`Estimated with Bureaucracy Buster: ${formatMinutes(timeEst.withMinutes)}`, margin, y, { size: 10, color: green });
  ...
  page.drawText("Generated by Bureaucracy Buster — Never auto-submits. Always review before filing.", {
    x: margin,
    y: 35,
    font,
    size: 7,
